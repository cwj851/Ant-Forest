...
files.ensureDir('/sdcard/森林梦/config_data/')
let dbFileName = '/sdcard/森林梦/config_data/Dingtalk_water.db'

//let dbFileName = fileUtils.getCurrentWorkPath() + '/config_data/Dingtalk_water.db'
/**
 * 能量收集数据
 */
let EnergyCollectData = {
  ...
  columnMapping: {
    ...
    FRIEND_ENERGY: ['friendEnergy', value => value, (cursor, idx) => cursor.getString(idx)],
    COLLECT_ENERGY: ['collectEnergy', value => value, (cursor, idx) => cursor.getString(idx)],
...
function AntForestDao () {
  ...
  this.saveFriendCollect = function (friendName, friendEnergy, collectEnergy, waterEnergy, collectTime) {
    ...
    //logUtils.debugInfo(['保存浇水数据：{}', JSON.stringify(sourceData)])
