...
module.exports = function (BaseHandler) {
  ...
  BaseHandler.addGroupToCheckTheCertificateGroups = (data, callbackId) => {
    if(config.CheckTheCertificateGroups.map(v => v.GroupName).indexOf(data.GroupName) < 0){
      config.CheckTheCertificateGroups.push({ GroupName: data.GroupName, WateringDate: '2022-01-01' })
      toast(data.GroupName + "加入查证列表成功！")
    }else{
      toast(data.GroupName + "已经在查证列表！")
    }
  }
