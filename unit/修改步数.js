 ...
 */
let accounts = config.huami_account_lists
let total = accounts.length
let sportResult = []
accounts.forEach((userInfo, idx) => {
  logUtils.debugInfo('准备修改账号[' + userInfo.username + ']的步数')
  generateFakeWalkStep(userInfo)
  if (idx < total - 1) {
    logUtils.infoLog('延迟十五秒钟执行下一个')
    sleep(15000)
  }
})

if (config.Sportpushplus && config.pushplus_token != '') { push_sport_today() }
logUtils.logInfo("====================");
logUtils.errorInfo("线程结束！修改步数已完成！")

function generateFakeWalkStep(userInfo) {
  let step = generateRandomStep(config.step_min, config.step_max)
  let result = fakeWalkData(userInfo.username, userInfo.password, step, device.getAndroidId())
  if (result) {
    logUtils.logInfo('账号' + userInfo.username + '修改步数成功 随机步数：' + step,true)
    sportResult.push({ 账号: userInfo.username, 状态: '修改成功！ [ ' + step + ' ]' })
  } else {
    logUtils.logInfo('账号' + userInfo.username + '修改步数失败',true)
    sportResult.push({ 账号: userInfo.username, 状态: '修改失败！' })
  }
}
...
function fakeWalkData(username, password, walkStep, deviceId) {
  ...
  if (accessToken == null) {
    logUtils.errorInfo('获取accessToken失败')
    return false
  }
  let loginResult = login(accessToken, deviceId)
  if (loginResult == null) {
    logUtils.errorInfo('登录失败')
    return false
  }
  ...
  function getAccessToken(username, password) {
    ...
    if (!regex.test(location)) {
      logUtils.errorInfo('请求access token失败')
      return null
    }
...
function push_sport_today() {
  logUtils.warnInfo('正在进行push+消息推送');
  if (parseInt(sportResult.length) > 0) {
    let push_data = { '总账号数': parseInt(sportResult.length), '修改详情': sportResult }
    let token = config.pushplus_token.replace(/ /g, '')
    let title = dateFormat(new Date(), 'yyyy-MM-dd') + '  运动步数'
