...
let DailyOnly = config.DailyOnly
let InviteWateringGroupList = config.InviteWateringGroupList
...
function InvitewateringEX(InviteWhiteList) {
      ...
      if (InviteWhiteList) {
        if (DailyOnly) {
          if (nickname.indexOf('WS') > -1) {
            if (!IsInWhiteList(InviteWhiteList, nickname)) {
...
function InviteWateringByGrouplists(InviteWateringGroupList) {
  for (var j = 0; j < InviteWateringGroupList.length; j++) {
    if (search_groupsAndclick(InviteWateringGroupList[j].groupName)) {
      ...
      var remindNum = InvitewateringEX(InviteWhiteList)
      closePage()
